...
const ManageEmployee = (): JSX.Element => {
  ...
  /**
   * function to get professional details with backend action
   *
   * @returns {void}
   */
  ...
  const columns = [
    ...
    // },
    {
      ...
      width: 90,
      renderCell: (params: UserProfileModel) => {
        console.log('super admin is', params.profile.is_super_admin);
        return (
          <>
            {!params.profile.is_super_admin && (
  ...
  useEffect(() => {
    ...
    // console.log(employeeProfessionalDetailsData);
    handleGetRoles();
  }, []);
